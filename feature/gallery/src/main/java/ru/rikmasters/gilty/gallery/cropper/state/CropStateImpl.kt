...
val CropState.cropData: CropData
    get() = CropData(
        ...
        pan = Offset(
            animatablePanX.targetValue,
            animatablePanY.targetValue
        ),
...
abstract class CropState internal constructor(
    ...
    internal open suspend fun updateProperties(
        ...
        if(
            ...
            // Set new zoom
            snapZoomTo(if(zoom > zoomMax) zoomMax else zoom)
    ...
    internal suspend fun animateTransformationToOverlayBounds(
        ...
        val zoom = zoom.coerceAtLeast(0.5f)
        
        // Calculate new pan based on overlay
        val newDrawAreaRect =
            calculateValidImageDrawRect(overlayRect, drawAreaRect)
        
        val newZoom = calculateNewZoom(
            oldRect = drawAreaRect,
            newRect = newDrawAreaRect,
            zoom = zoom
        )
     ...
     */
    private fun calculateValidImageDrawRect(
        rectOverlay: Rect, rectDrawArea: Rect,
    ): Rect {
        ...
        var rectImageArea =
            Rect(offset = rectDrawArea.topLeft, size = Size(width, height))
        
        if(rectImageArea.left > rectOverlay.left) {
            rectImageArea =
                rectImageArea.translate(
                    rectOverlay.left - rectImageArea.left,
                    0f
                )
        }
        ...
        if(rectImageArea.right < rectOverlay.right) {
            rectImageArea =
                rectImageArea.translate(
                    rectOverlay.right - rectImageArea.right,
                    0f
                )
        }
        ...
        if(rectImageArea.top > rectOverlay.top) {
            rectImageArea =
                rectImageArea.translate(
                    0f,
                    rectOverlay.top - rectImageArea.top
                )
        }
        ...
        if(rectImageArea.bottom < rectOverlay.bottom) {
            rectImageArea =
                rectImageArea.translate(
                    0f,
                    rectOverlay.bottom - rectImageArea.bottom
                )
        }
    ...
    internal fun getOverlayFromAspectRatio(
        ...
        if(aspectRatio == AspectRatio.Unspecified) {
            ...
            val overlayWidthMax =
                drawAreaWidth.coerceAtMost(containerWidth * coefficient)
            val overlayHeightMax =
                drawAreaHeight.coerceAtMost(containerHeight * coefficient)
    ...
    private fun getCropRectangle(
        ...
        drawArea: Rect, overlay: Rect,
    ) = if(drawArea == Rect.Zero || overlay == Rect.Zero)
        Rect(Zero, Size(bitmapWidth.toFloat(), bitmapHeight.toFloat()))
    else calculateValidImageDrawRect(overlay, drawArea).let {
        Rect(
            Offset(
                ((overlay.left - it.left) * (bitmapWidth / it.width)),
                ((overlay.top - it.top) * (bitmapHeight / it.height))
            ), Size(
                bitmapWidth * (overlay.width / it.width),
                bitmapHeight * (overlay.height / it.height)
            )
        )
    }
}
