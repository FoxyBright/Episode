...
class TranslationWebSource: KtorSource() {
    ...
    suspend fun getTranslationInfo(translationId: String) =
        tryGet("http://$HOST$PREFIX_URL/meetings/$translationId/translation")
    ...
    suspend fun sendMessage(translationId: String, text: String) =
        tryPost(
            "http://$HOST$PREFIX_URL/translations/$translationId/message"
        ) { url { query("text" to text) } }
