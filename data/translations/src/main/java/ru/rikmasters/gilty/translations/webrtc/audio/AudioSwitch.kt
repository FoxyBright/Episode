...
class AudioSwitch internal constructor(
    ...
    fun start(listener: AudioDeviceChangeListener) {
        audioSwitchLog("[start] state: $state")
     ...
     */
    fun stop() {
        audioSwitchLog("[stop] state: $state")
        when (state) {
            State.ACTIVATED -> {
                deactivate()
                closeListeners()
     ...
     */
    fun activate() {
        audioSwitchLog("[activate] state: $state")
        when (state) {
            State.STARTED -> {
    ...
    private fun deactivate() {
        audioSwitchLog("[deactivate] state: $state")
        when (state) {
            State.ACTIVATED -> {
    ...
    private fun selectDevice(audioDevice: AudioDevice?) {
        audioSwitchLog("[selectDevice] audioDevice: $audioDevice")
    ...
    private fun activate(audioDevice: AudioDevice) {
        audioSwitchLog("[activate] audioDevice: $audioDevice")
        when (audioDevice) {
    ...
    private fun enumerateDevices(bluetoothHeadsetName: String? = null) {
        audioSwitchLog("[enumerateDevices] bluetoothHeadsetName: $bluetoothHeadsetName")
        ...
        audioSwitchLog("[enumerateDevices] selectedDevice: $selectedDevice")
    ...
    private fun addAvailableAudioDevices(bluetoothHeadsetName: String?) {
        audioSwitchLog("[addAvailableAudioDevices] wiredHeadsetAvailable: $wiredHeadsetAvailable, " +
                "bluetoothHeadsetName: $bluetoothHeadsetName")
        mutableAudioDevices.clear()
        preferredDeviceList.forEach { audioDevice ->
            audioSwitchLog("[addAvailableAudioDevices] audioDevice: ${audioDevice.simpleName}")
            when (audioDevice) {
                ...
                AudioDevice.WiredHeadset::class.java -> {
                    audioSwitchLog("[addAvailableAudioDevices] #WiredHeadset; wiredHeadsetAvailable: $wiredHeadsetAvailable")
                ...
                AudioDevice.Earpiece::class.java -> {
                    ...
                    audioSwitchLog("[addAvailableAudioDevices] #Earpiece; hasEarpiece: $hasEarpiece, " +
                ...
                AudioDevice.Speakerphone::class.java -> {
                    ...
                    audioSwitchLog("[addAvailableAudioDevices] #Speakerphone; hasSpeakerphone: $hasSpeakerphone")
                    if (hasSpeakerphone) {
